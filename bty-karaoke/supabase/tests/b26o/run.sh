#!/usr/bin/env bash
# BUILD 26O — reproducible Postgres authority harness for pass issuance actor attribution.
#
...
# order, re-applies the BUILD 26O migration to prove idempotency, then runs the suite.
#
# Its own port and data directory (54351 / /tmp/pgk-b26o) so it can run alongside the b24/b25
...
#
#   bash supabase/tests/b26o/run.sh              # build, test, destroy
#   KEEP=1 bash supabase/tests/b26o/run.sh       # leave the cluster up for psql poking
set -euo pipefail

PORT="${PORT:-54351}"
PGDIR="${PGDIR:-/tmp/pgk-b26o}"
...
$PSQL -f "$HERE/00_prereq_b26o.sql"
...
# Re-running the BUILD 26O migration must be a no-op (ordered AND idempotent). This catches a
...
PGPORT="$PORT" node "$HERE/issuance-attribution.pg.test.mjs"

# The regression half. BUILD 26O republishes the issuance RPC and constrains the audit table —
