#!/usr/bin/env npx tsx
/**
 * PREREQUISITE-AUTHORITY REPLAY RUNNER BUILDER (Slice 3.2I-R5B1A.1-R2.48 Part 12).
 *
 ...
 * PREPARED in R2.48, deliberately NOT EXECUTED.
 *
 *   npx tsx scripts/practice-c18-prerequisite-authority-replay-runner.ts --out /tmp/r248_c18_prerequisite_authority_replay_canary.sh
 *   npx tsx scripts/practice-c18-prerequisite-authority-replay-runner.ts --binding-json
 */
...
# =============================================================================
# BTY Practice — R2.48 PREREQUISITE-AUTHORITY + EMPTY-POOL REPLAY CANARY
# Slice 3.2I-PRACTICE-R5B1A.1-R2.48
#
...
#
#   The R2.46 run reached the provider twice and produced no verdict. R2.47 read
#   both DTOs and found the reviewer contract stating one axis twice,
#   incompatibly: the prompt said candidate selection is "decided by the LIST,
#   not by your status" and showed a governed-action-only table, while the truth
#   -state table forbids prerequisite candidates by STATE regardless of pool.
#   The model followed the principle; three rows cited a forbidden failure
#   candidate; the run terminated.
#
#   R2.48 scoped the list rule to the governed action, generated every per-state
#   clause from the requirement fields themselves, and made a required-but-empty
#   prerequisite pool fail closed per role. Retrying before those landed could
#   not have succeeded — the contract contradicted itself.
#
# WHAT THIS RUN IS FOR
#
#   1. Obtain a MERGED verdict on the corrected contract. Deterministically the
#      corrected merge yields 12 surfaces, two packet items, primary[1] owning
#      the branch defect. This checks that holds when the rows come from the
#      model rather than from a fixture.
#
...
#   boundary_candidate_forbidden_present still fires on a non-empty pool. No
#   polarity-invalid candidate is restored (57 candidates, byte-identical map).
#   The JSON schema is unchanged. R2.46 attribution, grouping, ownership and
#   packet dedup are untouched, and primary[1]'s direct row is still its own.
#
# STILL OPEN, DELIBERATELY
#   The three APPLICABILITY false positives remain unaddressed by any rule. A
#   green run here is NOT a product-quality pass and must not be reported as one.
#
# =============================================================================
set -Eeuo pipefail
...
BINDING_JSON="$(npx --yes tsx scripts/practice-c18-prerequisite-authority-replay-runner.ts --binding-json)" \\
