#!/usr/bin/env npx tsx
/**
 * FIELD-REPAIR REPLAY RUNNER BUILDER (Slice 3.2I-R5B1A.1-R2.50 Part 14).
 *
 ...
 * PREPARED in R2.50, deliberately NOT EXECUTED.
 *
 *   npx tsx scripts/practice-c18-field-repair-replay-runner.ts --out /tmp/r250_c18_field_repair_replay_canary.sh
 *   npx tsx scripts/practice-c18-field-repair-replay-runner.ts --binding-json
 */
...
# =============================================================================
# BTY Practice — R2.50 FIELD-LEVEL PATCH REPAIR REPLAY CANARY
# Slice 3.2I-PRACTICE-R5B1A.1-R2.50
#
...
#
#   R2.48 rewrote the reviewer contract and the repair response came back
#   BYTE-IDENTICAL to R2.46's on all nine shared surfaces. R2.49 proved the
#   prompt was never the operative variable: the repair re-asked whole SURFACES,
#   so a row invalid in ONE field was re-solved in all six. Four rows had already
#   answered prerequisiteFailureCandidateId 'none' correctly; re-asking re-opened
#   the correct field and the model filled it with a true inherited fact the
#   state forbids. The run lost its verdict to an improvement.
#
#   R2.50 replaces whole-row repair with a patch. The second call carries only
#   {surfaceRef, field, value} operations against a server-owned plan. A field
#   the validator accepted is not in the plan, so sending it is refused; frozen
#   values are structurally ABSENT from the exchange.
#
# WHAT THIS RUN IS FOR
#
#   1. Obtain a MERGED verdict. Deterministically the captured attempt-1 rows
#      need exactly 13 operations across 10 surfaces in 10 dependency groups,
#      and the patched matrix completes. Check that holds when the operations
#      come from the model rather than from a fixture.
#
#   2. RE-TAKE the applicability measurement on a run that reaches a verdict.
#      Record the number. Do not act on it here.
#
...
#   complete semantic measurement, NOT a repair failure, and must not be
#   reported as one.
#
# WHAT R2.50 DOES NOT CHANGE
#   The initial full-row DTO and its provider schema. Candidate pools (57,
#   byte-identical map). R2.44 polarity, R2.46 attribution and packet dedup,
#   R2.48 prerequisite authority and empty-pool guards. forbidden_present still
#   fires. No forbidden candidate is ever normalized to 'none'.
#
...
BINDING_JSON="$(npx --yes tsx scripts/practice-c18-field-repair-replay-runner.ts --binding-json)" \\
