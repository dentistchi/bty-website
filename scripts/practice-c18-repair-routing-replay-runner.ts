#!/usr/bin/env npx tsx
/**
 * REPAIR-ROUTING REPLAY RUNNER BUILDER (Slice 3.2I-R5B1A.1-R2.52 Part 14).
 *
 ...
 * PREPARED in R2.52, deliberately NOT EXECUTED.
 *
 *   npx tsx scripts/practice-c18-repair-routing-replay-runner.ts --out /tmp/r252_c18_repair_routing_replay_canary.sh
 *   npx tsx scripts/practice-c18-repair-routing-replay-runner.ts --binding-json
 */
...
# =============================================================================
# BTY Practice — R2.52 REPAIR-ROUTING REPLAY CANARY
# Slice 3.2I-PRACTICE-R5B1A.1-R2.52
#
...
#
#   The R2.50 canary passed every check and the live run still took the LEGACY
#   whole-row repair. R2.51 proved why: the repair dependency was optional and
#   no caller supplied it, so the stage fell through to a second full-row call.
#   reviewFieldRepair had zero importers. Every binding was a CONTENT digest,
#   and a dead module satisfies those exactly as well as a live one.
#
#   This canary therefore proves REACHABILITY, without a credential:
#     - a FORCED-INCOMPLETE mock leg reproduces the retained R2.48 attempt 1
#       through the same entrypoint and dependency construction the live path
#       uses, then reads the artifact back: repairMode field_patch, full-row
#       calls 1, patch calls 1, legacy calls 0, response repairs[] with no
#       assessments, 13 operations across 10 dependency groups, 12 surfaces,
#       zero frozen mutations, and no legacy failed-subset event;
#     - STATIC checks prove the dependency is required, the entrypoint and the
#       production service both supply it, and the whole-row fallback is gone.
#
#   A complete first-pass mock is NOT sufficient and is no longer the only leg.
#
...
BINDING_JSON="$(npx --yes tsx scripts/practice-c18-repair-routing-replay-runner.ts --binding-json)" \\
...
# ---------------------------------------------------------------------------
# R2.52 FORCED-INCOMPLETE MOCK LEG
#
# A complete first-pass mock can never exercise repair routing, which is exactly
# how R2.51's defect survived every preflight: the second live call was the
# LEGACY whole-row re-ask while every check passed. This leg forces the retained
# R2.48 attempt-1 shape through the SAME entrypoint and dependency construction
# the live path uses, and proves which route the second call took.
# ---------------------------------------------------------------------------
printf '\\nFORCED-INCOMPLETE REPAIR ROUTING PROOF (no credential, no network)\\n'
ROUTE_DIR="$(mktemp -d)"
...
BTY_C18_NARROW_MOCK=1 npx --yes tsx scripts/practice-c18-narrow-boundary-replay.ts \\
  ...
  || wiring_failed 'the forced-incomplete mock leg failed to run'

ROUTE_ARTIFACT="$(find "$ROUTE_DIR" -maxdepth 1 -name 'practice-review.boundaryreplay.mock.*.json' | head -1)"
[ -n "$ROUTE_ARTIFACT" ] || wiring_failed 'the forced-incomplete leg wrote no artifact'
...
route_check 'operation count'                  'b["fieldRepairMetrics"]["fieldRepairOperationCount"]' '13'
...
route_check 'written artifact version'         'b["artifactVersion"]'                              '"practice-narrow-boundary-replay/5"'
...
printf '\\nREPAIR ROUTING PROOF PASS - SECOND CALL WAS A PATCH - LEGACY WHOLE-ROW UNREACHABLE\\n'
