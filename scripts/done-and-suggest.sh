...
#
# 사용: cd ~/Dev/btytrainingcenter && ./scripts/done-and-suggest.sh
...
suggest_commit() {
  ...
  local tag="chore"
  if [[ -f "$SUMMARY" ]]; then
    task_line="$(grep -E "^Task Line:" "$SUMMARY" 2>/dev/null | sed 's/^Task Line:\s*//' | head -1)"
