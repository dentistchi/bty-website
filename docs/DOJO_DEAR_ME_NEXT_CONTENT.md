...
**갱신**: 2026-03-06 — 50문항·연습 플로우 2종 설계 갱신은 `DOJO_DEAR_ME_50_AND_2FLOWS_1PAGE.md` 참고(구현 상태·콘텐츠 심화 §0·§5). **50문항 콘텐츠 확장 설계(목차·플로우)** → `DOJO_DEAR_ME_50_CONTENT_EXPANSION_1PAGE.md` 참고.  
...
## 7. 50문항 목차·연습 플로우 2~5단계 스펙 정리 (§1-4·§6·§4·§5 기준)

**단일 참조**: 아래 표는 §1-4, §6, §4, §5를 한 곳에 모은 스펙 정리. 도메인·API는 진입(entry)·제출(submit)·결과(result)만 비즈니스 규칙으로 둠.

### 7-1. 50문항 목차 (Dojo 분석)

| 영역 | 문항 수 | 설명 (스펙) | 입력 형식 |
|------|---------|-------------|-----------|
| 역지사지 | 10 | 상대 입장 이해·역할 뒤집기 관련. | 5단계 리커트 |
| 소통·경청 | 10 | 경청·질문·피드백 수용. | 5단계 리커트 |
| 리더십·책임 | 10 | 결정·위임·피드백 주기. | 5단계 리커트 |
| 갈등·협상 | 10 | 대립·타협·윈윈. | 5단계 리커트 |
| 팀·협업 | 10 | 팀 목표·역할·신뢰. | 5단계 리커트 |
| **총** | **50** | 문항 진행: 한 번에 10문항 또는 전체. | 제출 후 결과(영역별 점수) + Dr. Chi 코멘트 |

**진입·제출·결과**: 진입(§4) → 문항 진행 → 제출(필수 50문항 응답) → 결과(영역별 점수·멘토 코멘트). 도메인: `src/domain/dojo/flow.ts`(canEnterDojo, validateDojo50Submit, computeDojo50Result).

### 7-2. 연습 플로우 2~5단계 (역지사지 연습 1종)

| 단계 | 내용 (스펙) | API/도메인 |
|------|-------------|------------|
| **2. 안내** | 역지사지 연습 목표·방법 1~2문장. | 화면만. |
| **3. 시나리오** | 단일 상황 제시 + 선택지(텍스트 또는 선택). | 제출: 상황 텍스트 또는 선택 ID. 도메인 `validateIntegritySubmit`. |
| **4. 피드백** | Dr. Chi 관점 코멘트. | `/api/mentor` 또는 Dojo 전용 엔드포인트. 도메인에서 결과/코멘트 키만 반환 시 API가 LLM 호출. |
| **5. 완료** | "오늘의 연습 완료" + (선택) 다음 연습 또는 멘토로. | 도메인 `completeIntegritySession`(완료 플래그)·XP 연동은 API. |

**진입·1단계**: §4 — `/bty` entryIntro + startCta → `/[locale]/bty/mentor`(훈련 선택) → 역지사지 시 `/bty/integrity`.

### 7-3. 진입·제출·결과 도메인 (비즈니스 규칙만)

| 구분 | 함수(안) | 설명 |
|------|----------|------|
| 진입 | `canEnterDojo(isAuthenticated)` | Dojo/역지사지 진입 가능 여부. (현재: 인증만) |
| 50문항 제출 | `validateDojo50Submit(answers)` | 응답 50건·1~5 리커트 범위 검증. |
| 50문항 결과 | `computeDojo50Result(answers)` | 5영역별 점수(0~100)·요약 키. UI/API는 도메인 결과만 표시. |
| 역지사지 제출 | `validateIntegritySubmit(payload)` | 시나리오 입력(텍스트 또는 선택) 필수 검증. |

- **구현 위치**: `src/domain/dojo/flow.ts` (순수 함수만). API는 검증·도메인 호출·응답만.
