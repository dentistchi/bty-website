...
- **SPRINT 238:** LE Stage run/result payload = `leStageDisplayForArenaRunResult`; leaderboard sort must ignore season/core identity fields; **AIR bands (v2 LOCKED):** low &lt; **0.50**, mid **0.50–0.79**, high **≥ 0.80** — `AIR_BAND_LOW_MID` / `AIR_BAND_MID_HIGH` in `air.ts`; forced-reset AIR condition uses **&lt; 0.80** (below high band), reason `air_7d_below_high_band_two_consecutive_weeks`. Healing awakening path = `isValidHealingAwakeningActId` + `nextHealingAwakeningActAfter` (`healing.ts`).
