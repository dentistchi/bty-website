...
- [ x] 로그인 계정 확인: ikendo1@gmail.com 로그인 시 대시보드/헤더에 표시되는 코드네임이 “FORGE · Inferno”인지 확인.
...
**구현 상세 (개발자 모드)**: DB·API·리더보드·프론트 레이어 합성까지 한 번에 정리한 명세는 **`docs/AVATAR_LAYER_SPEC.md`**. 브레인스토밍 축 5개, 마이그레이션, DTO, `AvatarComposite` 컴포넌트, Cursor 1·3·4 복사용 프롬프트 포함. 이 §3은 비전·요구사항, 실제 구현은 AVATAR_LAYER_SPEC 따름.

### 3.1 목표

- **캐릭터(얼굴·몸):** 한 번 선택·저장하면 다음 “Code 진화” 전까지 **고정** (이미 구현된 `avatar_character_locked` 정책 유지).
- **시각 요구 (확정):** 얼굴과 몸을 따로 만들고(레이어 분리), 옷 선택 시 옷 이미지가 보여야 하며 그 옷을 입은 캐릭터가 표시되어야 함. 대시보드·리더보드에서 캐릭터+선택한 옷 함께 노출.
- **옷:**  
...
### 3.3-1. 프로그램 합성 vs 미리 조합 파일 — 권장

| 방식 | 장점 | 단점 |
|------|------|------|
| **프로그램으로 합성** (레이어 이미지를 런타임에 겹침) | 조합 수만큼 파일을 만들 필요 없음. 캐릭터 N명 × 옷 M벌 = **레이어 파일 N+M개**만 있으면 됨. 새 옷 추가 시 **옷 1개 파일**만 추가. | 레이어 소스(얼굴·몸·옷별 PNG)를 맞춰 제작해야 함. CSS/Canvas 합성 로직 필요. |
| **조합을 미리 파일로 제작** | 구현 단순. URL 하나만 반환하면 됨. 품질을 제작 단계에서 통제 가능. | 캐릭터 4명 × 옷 10벌 = **40개 이미지**. 새 옷 추가 시 캐릭터 수만큼 새 파일. 조합이 늘수록 제작·저장 비용 증가. |

**권장: 프로그램 합성(레이어 방식, 위 선택지 B)**  
- **레이어 파일만** 미리 만든 뒤(캐릭터 베이스 1장, 옷별 1장, 투명도 맞춤), **런타임에 겹쳐서 표시**하는 방식을 추천.
- 이유: 옷·캐릭터가 늘어나도 **파일 수가 선형**으로만 늘어나고, 새 옷 추가 시 옷 이미지 1장만 추가하면 됨. 미리 조합 파일은 조합 수가 곱해지므로 확장 시 비용이 큼.
- 전제: 아트가 **레이어로 나눠 제작 가능한지**(캐릭터 베이스 + 옷 레이어 분리). 불가능하면 당분간 **C(미리 조합)** 로 소규모 조합만 제작하는 것도 가능.

### 3.3-2. A vs B vs C — 차이·의견·권장

**A와 B의 차이**

| 구분 | A (서버 합성) | B (프론트 레이어 합성) |
|------|----------------|-------------------------|
| **합성 시점** | **서버**에서 캐릭터+옷 이미지를 겹쳐서 **새 이미지 1장 생성** | **브라우저**에서 캐릭터 URL·옷 URL을 **순서대로 겹쳐서 표시** (이미지 생성 없음) |
| **API가 주는 것** | 합성된 **이미지 URL 1개** | **캐릭터 베이스 URL + 옷 URL** (및 필요 시 악세사리 ID 목록) |
| **클라이언트** | `<img src={url} />` 만 하면 됨 | CSS로 여러 `<img>`를 겹치거나, Canvas에 순서대로 그리기 |
| **서버 부담** | 이미지 처리(sharp/canvas 등) 필요. 조합별 캐시 저장 권장 | 없음. 기존 정적/스토리지 URL만 반환 |
| **확장(악세사리 추가)** | 서버 합성 로직에 레이어 하나 더 추가, 캐시 키 확장 | 클라이언트에 레이어 하나 더 쌓기. API에 URL 하나 더 내려주면 됨 |

**C(미리 렌더 조합)**  
- 조합마다 **미리 제작된 파일 1개**. API는 조합 키(캐릭터+옷)에 맞는 **URL 1개**만 반환.  
- 구현 난이도는 가장 낮지만, 조합 수가 곱해지므로 확장 시 제작·저장 비용이 가장 큼.

---

**의견: 구현 가능성 + 나중에 확장하기에 가장 적합한 것**

- **가장 구현·확장에 적합한 선택: B (프론트 레이어 합성)**  
  - **이유 1 (구현)**  
    - 서버에 이미지 합성 파이프라인(sharp/캐시/스토리지)을 둘 필요가 없음.  
    - API는 이미 있는 `avatar_character_id`, `avatar_selected_outfit_id`(또는 outfit URL)만 내려주면 되고, 프론트는 `avatarOutfits.ts` 등에서 **캐릭터 베이스 URL + 옷 URL**만 알면 됨.  
    - 레이어 소스(캐릭터 베이스 PNG, 옷별 PNG, 크기·투명도 맞춤)만 준비하면 CSS 겹침 또는 Canvas로 바로 구현 가능.  
  - **이유 2 (확장)**  
    - 새 옷 추가 = 옷 이미지 1장 + 옷 ID/URL 매핑만 추가.  
    - 악세사리 추가 = 레이어 순서만 정하고 URL 하나 더 내려주면 됨.  
    - 서버 합성(A)처럼 “새 레이어 타입 추가 → 캐시 키·재생성 정책”을 신경 쓸 필요 없음.  
  - **이유 3 (운영)**  
    - 합성 결과 캐시·스토리지 용량·만료 정책 등이 없어서 운영이 단순함.

- **A(서버 합성)** 는  
  - “단일 이미지 URL”이 꼭 필요할 때(OG 이미지, 이메일, 매우 제한된 클라이언트) 유리.  
  - 대시보드·리더보드는 앱 내부 뷰이므로, 지금 단계에서는 B만으로 충분하고, 나중에 공유/OG용으로만 A를 추가하는 식이면 됨.

- **C(미리 조합)** 는  
  - 캐릭터·옷 수가 적고(예: 2~3캐릭터, 2~3옷) 앞으로도 거의 늘리지 않을 때만 비용이 맞음.  
  - 확장을 염두에 두면 B가 유리함.

**정리**  
- **대시보드·리더보드에 캐릭터+선택한 옷이 함께 나오게** 할 때는 **B(프론트 레이어 합성)** 로 구현하는 것을 권장.  
- API: 프로필/리더보드 응답에 **캐릭터 베이스 이미지 URL**, **선택한 옷 이미지 URL** (및 필요 시 악세사리)을 내려주고, 클라이언트에서 동일한 레이어 순서로 겹쳐서 표시.
...
## 9. Cursor 4 할일 — 아바타 얼굴·몸 분리 + 옷 입히기 (복사용 프롬프트)

**상세**: 본 문서 §3.1(시각 요구), §3.2~§3.4(구조·표시 방식 A/B/C·단계별 계획).

| Cursor | 복사용 프롬프트 |
|--------|------------------|
| **Cursor 4** | `docs/ARENA_CODENAME_AVATAR_PLAN.md` §3 반영해줘. 캐릭터 **얼굴과 몸**을 레이어로 따로 두고, **옷을 선택했을 때** 옷 이미지가 보이면서 **그 옷을 입은 캐릭터**가 표시되도록 해줘. §3.3 표시 방식 A(서버 합성) / B(프론트 레이어 합성) / C(미리 렌더 조합) 중 구현 가능한 하나 골라서, 대시보드·리더보드에 캐릭터+선택한 옷이 함께 나오게 구현해줘. `avatar_character_id`, `avatar_selected_outfit_id`, `avatarOutfits.ts`, profile PATCH·core-xp·리더보드 아바타 표시 경로 확인. |
