...
- [x] 2026-03-30 — **MyPageLeadershipConsole load/auth** — `load()` uses `fetch` + 401 single retry + `userId` guard + structured `console.error`/`warn`; `loadError` retry UI + `isLoading` hub skeleton; refetch flag waits on `sessionUserId`; Vitest `MyPageLeadershipConsole.test.tsx` · **작업 완료**
