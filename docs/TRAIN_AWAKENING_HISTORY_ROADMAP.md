...
적용 표면: /train/day/[day] (page.client.tsx) 단일.
미정정: C1+C2 ledger (outer docs-only, 묶어서 1회 정정 예정).
...
- (B) Awakening 미완료 시 client-side exception — awakening?_rsc / second-awakening / progress fetch 후 렌더 터짐. 1?_rsc=150ah 중복 호출 관측. → 요청 #8은 진짜 버그. 우선순위 ↑.
...
| 8 | Awakening 미완료 시 에러 (client-side exception) | 버그 수정 | MED | 낮음 | 예(에러 원인·progress null 가드) | 테스터 차단 → 우선 ↑ |
...
4. #8 Awakening 미완료 에러 (progress null 가드 등)
5. #7 Live ranking 잘림
...
- C1+C2 ledger 정정 (outer docs-only) — 언제든 가능, Phase A 끝에 묶어도 됨.
