...
지시: 아래 파일을 **지금 열어서** 내용을 확인한 뒤 행동해.
- 열 파일: **docs/CURSOR_TASK_BOARD.md**
- "이번 런" 제목 아래 **TASK 1~10 표**에서 OWNER **C3** 행만 봐.
- **C3 OWNER 행에 `[ ]`가 하나도 없으면** (예: TASK8·9 모두 [x]) → **"C3 할 일 없음. CONTINUE(C3) 여기서 중단. 다음 C3 태스크는 C1이 SPRINT 82 등 새 이번 런을 열 때 생김."** 만 답하고 종료. **코드·도메인 변경 없음.**
- C3 행 중 **`[ ]`가 있으면** 그 TASK만 수행.
- 표에 First Task 잠금이 있으면, TASK 1(C5)이 [ ]이면 "TASK 1 완료 대기. Exit." 가능.

다른 파일/예전 배치/`SPRINT_PLAN`의 REFRESH 5줄 말고 **위 표의 OWNER C3 열만** 기준으로 해. (같은 런에서 `[ ]`로 남은 것은 보통 **C1 DOCS** 뿐.)
```
