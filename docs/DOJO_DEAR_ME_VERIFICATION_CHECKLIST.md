# Dojo·Dear Me 2차 구현 검증 체크리스트

**용도**: `docs/DOJO_DEAR_ME_NEXT_CONTENT.md` 2차 구현 완료 후, 다른 커서(검증 에이전트)가 확인할 때 사용.  
**참고**: DOJO_DEAR_ME_NEXT_CONTENT.md §4(2차 구현 요약), §2-2(Dear Me 진입).

**2차 전체 검증(항목 1~10, 50문항·연습 플로우 2~5단계·규칙 포함)**: **`docs/DOJO_DEAR_ME_2ND_VERIFICATION.md`** 를 사용하세요. 해당 문서에 체크리스트·검증 결과 템플릿이 있음.
...
| 5 | **/dear-me 첫 화면 진입 문구·CTA** | `/en/dear-me`, `/ko/dear-me` 접속 시 `todayMe.entryIntro` + `todayMe.startCta`("시작하기")가 보이고, 클릭 시 본문(자존감 진단·안전한 거울 등) 또는 다음 단계가 노출되는지 | |
| 6 | **i18n: Dear Me 진입 ko/en** | `todayMe.entryIntro`, `todayMe.startCta`가 ko·en 모두 정의되어 있는지 | |
| 7 | **locale 전달** | `/bty`·`/dear-me` 페이지에서 locale이 하위 컴포넌트·i18n에 올바르게 전달되는지 (en/ko 전환 시 문구 변경) | |
...
## 검증 결과 — 옵션 A (Dear Me 1차 플로우 진입, 단계 1)
...
## 검증 결과 — Dojo·Dear Me 2차 전체 (재검증)

| 항목 | 내용 |
|------|------|
| **검증 일시** | 다른 커서가 Dojo·Dear Me 2차(옵션 A) 구현 완료 후 Cursor 2 검증. |
| **스펙 대조** | DOJO_DEAR_ME_NEXT_CONTENT §4·§5·§6·§2-2: 진입(entryIntro+startCta), Dojo → mentor, Dear Me → 본문 노출, 50문항/연습 플로우는 문서 목차·스펙만(코드 미구현 범위). |
