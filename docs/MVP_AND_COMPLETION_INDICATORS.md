...
### 3.1 어떤 %를 말할 때 무엇을 쓰나

두 가지 지표가 있으며 **서로 바꿔 쓰지 않는다.**

1. **제품 기능 완료율 (Phase 기준)** — **본 문서만** 기준으로 한다. **100%**는 **Phase 4** 체크리스트 전 항목 완료(§2 마지막 행). 현재 %는 팀이 고정한 목표 Phase(1~4)에 대해 §5~§7 체크리스트와 §4 가중치·§9 수식으로 산출한다. **대외·기획에서 “제품이 몇 %냐”**고 물을 때는 이 지표를 쓴다.

2. **프로젝트 전체 진척 (500점 모델)** — **docs/PROGRESS_TRACKER.md**: 서류·코드·배포를 점수화하고, 코드 하위에 Center·Arena·Foundry·동영상·애니메이션 제작까지 포함한다. **docs/PROGRESS_SNAPSHOT.md**는 이 500점 합산을 한눈에 보여 줄 때 쓴다(“500점 모델” 요약).

3. **관계:** “제품 스펙만” → Phase 기반 %. “문서·자산·배포까지 포함한 프로젝트 진척” → 500점 %. 스냅샷의 %가 Phase %와 다를 수 있다는 것이 정상이다.
...
- **"진행률이 얼마야?" (500점 모델)** → **docs/PROGRESS_SNAPSHOT.md** (Center/Arena/Foundry/전체 % 한눈에). **Phase 기준 제품 %**는 위 §3.1과 본 문서 체크리스트로 별도 산출.
- **"다음에 뭘 해야 해?"** → **docs/ROADMAP_IMPLEMENTATION.md** (구현 순서).
