...
export const REVIEW_SYSTEM_PROMPT: string = [
  ...
  "nextDecisionDimension FORM — write it as something actually BEING DECIDED, never as a topic label. In English phrase it with what / when / who / whom / how much / how many / whether / which / how to. In Korean use a decision-question form such as 무엇을 / 언제 / 누가 / 누구에게 / 어느 정도 / 얼마나 / 어떻게 / -(으)ㄹ지. 'client communication' is a topic and is NOT acceptable; 'when to update the client' is a decision and is acceptable. This is a FORM requirement only: it constrains how you write the dimension, never which dimension is correct.",
  ...
  "Record your impression of sibling similarity in overlapsOtherBranchIndex, overlapReason and branchDistinct: the index of a sibling branch you think poses substantially the same next decision, otherwise -1. This is RECORDED FOR HUMAN REVIEW and does not by itself reject the scenario, so report what you observe and do not strain to find sameness or to avoid it.",
  "",
  "",
  ...
  "construction.boundaryCompliance is a generator claim / declared coverage reference, never proof of semantic compliance. Independently judge the visible label and action against every confirmed boundary; a claim cannot make an absent, decorative, or violating boundary compliant.",
  "",
  ...
  "",
  "CROSS-BRANCH CAUSAL DIVERSITY — each branch must be the consequence of a DIFFERENT primary choice. For each branch state selectedPrimaryEffect, affectedStakeholders, resourceOrRelationshipChange, causalLink, boundaryState and urgencyState. Then in crossBranch report overlapping resulting worlds, overlapping next-decision axes, overlapping stakeholders and repeated action meanings as index pairs like '0-1'. Shared stakeholders belong in stakeholderOverlapPairs, which is recorded rather than penalised.",
  "nextDecisionAxisOverlapPairs is a free-text note recorded for human review; it does not by itself reject the scenario.",
  "Set branchesInterchangeable=true when branch content could be swapped between primary choices without becoming incoherent. Set allBranchesSameGenericAxis=true ONLY when every branch poses substantially the SAME decision variable — for example every branch asking nothing but whether to disclose. Branches deciding DIFFERENT variables inside one shared topic are not one generic axis, so do not set it merely because every branch concerns communication.",
  "A SHARED STAKEHOLDER OR A SHARED TOPIC IS NOT A DEFECT: the same client, manager or subject matter may appear in every branch when the causal state and the DECISION VARIABLE genuinely differ. Do not demand vocabulary variety; demand a different decision.",
  "",
