...
export const NARROW_BOUNDARY_SYSTEM_PROMPT: string = [
  ...
  "",
  "  GOVERNED-ACTION CANDIDATE — controlled by the LIST. Selection here is decided by the cardinality of `governedActionCandidates`, not by the status you choose:",
  "",
  ...
  "  When that list is NOT empty you must select exactly one id from it — whether you answer `present` or `absent`. Selecting on an `absent` row is not a contradiction: the candidate records what the surface DOES, and by pairing it with `absent` you are saying that the action you selected is not the action the boundary governs.",
  "  Use `uncertain` only when the text is genuinely ambiguous — never to avoid choosing from a list.",
  "",
  "  PREREQUISITE CANDIDATES — controlled by the STATE, never by the list. This rule is the opposite of the one above and does not inherit from it:",
  "  Which prerequisite ids you may cite is fixed by the chosen truth state, and a non-empty list never licenses a citation the state does not ask for.",
  "    - A state that FORBIDS a prerequisite role requires `none` for it, even when that candidate list is non-empty. A prerequisite fact that is true of the surrounding context is irrelevant to a surface whose state does not use that role — do not record it there.",
  "    - A state that REQUIRES satisfaction evidence must cite a `prerequisiteSatisfactionCandidates` member; a state that REQUIRES failure evidence must cite a `prerequisiteFailureCandidates` member.",
  "    - If the list a state REQUIRES is empty, that state is UNSUPPORTED for this surface: do not select it, and do not substitute the sentinel. An empty list means the server found nothing on this surface pointing that way, so the claim cannot be made here. Choose the state the evidence you actually have supports.",
  "  Each state below names its own three requirements. Follow that line; it is generated from the same table the server validates against.",
  "",
...
/**
 * One complete legal shape for one dependency group, as the model sees it.
 *
 * Flat and fully enumerated on purpose. R2.53 measured a group published as five independent scalar
 * lists whose product admitted 150 combinations, only a small canonical subset of which formed a
 * valid state — and the one the model picked was refused for a reason no list mentioned.
 */
export type FieldRepairGroupAlternativeView = {
  ...
  /** `must_be_empty` | `model_required` — the reason authority, carried BY the alternative. */
  reasonMode: string;
};

export type FieldRepairDependencyGroupView = {
  groupId: string;
  surfaceRef: string;
  /** Answer all of these, from ONE alternative. */
  fields: string[];
 ...
 *
 * R2.54 — a grouped target publishes NO `allowedValues`. Sending one would re-open the door this
 * slice closed: the model would read five independent lists, pick a plausible value from each, and
 * the tuple it assembled would be nobody's canonical state. The group's shapes live in
 * `dependencyGroups`, once per group rather than repeated on every member.
 */
export function buildFieldRepairRequest(subject: NarrowBoundarySubject, plan: FieldRepairPlan): FieldRepairRequest {
  const groups = new Map<string, FieldRepairDependencyGroupView>();
  for (const t of plan.targets) {
    if (t.valueAuthority !== "canonical_group_alternative" || groups.has(t.groupId)) continue;
    groups.set(t.groupId, {
      groupId: t.groupId,
      surfaceRef: t.surfaceRef,
      fields: [...t.groupFields],
      alternativesSha256: t.alternativesSha256,
      alternatives: t.alternatives.map((a) => ({
        alternativeId: a.alternativeId,
        stateId: a.stateId,
        prerequisiteStatus: a.prerequisiteStatus,
        temporalRelation: [...a.temporalDomain],
        prerequisiteSatisfactionCandidateId: [...a.satisfactionCandidateDomain],
        prerequisiteFailureCandidateId: [...a.failureCandidateDomain],
        reasonMode: a.reasonConstraint,
      })),
    });
  }
  return {
