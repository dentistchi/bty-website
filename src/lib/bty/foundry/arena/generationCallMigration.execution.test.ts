...
const BASEV = `values ('${ATTEMPT}', 'generation', @G, @K, 'gpt-4o-mini', 120000, 'json_schema_strict'`;
let seq = 0;
/** Each fixture gets a fresh sequence pair so uniqueness never masks the property under test. */
const call = (cols: string, vals: string) => {
  seq += 1;
  return `${BASE}${cols}) ${BASEV.replace("@G", String(seq)).replace("@K", String(seq))}${vals});`;
};
...
describe.runIf(Boolean(PG_BIN))("[R5C-2A] the lifecycle is enforced by the database", () => {
  ...
  it("enforces BOTH sequence uniqueness constraints", () => {
    psql(["-tAc", `insert into ${CALLS_TABLE} (attempt_id, call_kind, global_sequence, kind_sequence, model, provider_timeout_ms, structured_output_mode)
      values ('${ATTEMPT}', 'boundary_review', 60, 60, 'm', 1, 'none');`], DB);
    // same attempt + same global sequence
    expect(ok(`insert into ${CALLS_TABLE} (attempt_id, call_kind, global_sequence, kind_sequence, model, provider_timeout_ms, structured_output_mode)
      values ('${ATTEMPT}', 'semantic_review', 60, 61, 'm', 1, 'none');`)).toBe(false);
    // same attempt + same kind + same kind sequence
    expect(ok(`insert into ${CALLS_TABLE} (attempt_id, call_kind, global_sequence, kind_sequence, model, provider_timeout_ms, structured_output_mode)
      values ('${ATTEMPT}', 'boundary_review', 61, 60, 'm', 1, 'none');`)).toBe(false);
  });
