...
function systemPrompt(locale: "en" | "ko"): string {
  ...
  return [
    ...
    "- completion_question: ONE question a participant answers after the material. It must be RELEVANT to the approved observable behavior but must NOT quote or paraphrase it as the answer \u2014 a participant who has just read the behavior must still have to supply something of their own. Ask for THEIR decision or THEIR concrete application ('what will you do differently the next time this happens?'), never for the behavior back ('describe how you will follow the standard'). NOT a generic template, a yes/no question, a feelings prompt, or a summary request. Never claim the behavior is already competent, permanent, or that trust/understanding was achieved.",
