...
/**
 * Team-level AIR/MWD aggregates + default TSP for a calendar window [start, end).
 * Individual AIR is never returned; only team averages.
 */
export async function fetchTeamTIIInputsForWindow(
...
export function buildGetTeamTIIInputs(
  ...
  return async (teamId: string, _weekStart?: string): Promise<TIIInputs> =>
    fetchTeamTIIInputsForWindow(admin, teamId, start, end);
}

/**
 * Rolling 7d window ending at `windowEnd` (e.g. now). Used when recomputing TII after an AIR write.
 */
export function buildRolling7dGetTeamTIIInputs(
  admin: SupabaseClient,
  windowEnd: Date
): GetTeamTIIInputs {
  const end = windowEnd;
