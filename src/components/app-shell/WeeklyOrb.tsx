...
import { mergeAmount, absorbHi as absorbHiAt, nodeGatherScale } from "@/domain/daily/weeklyOrbMerge";
...
export default function WeeklyOrb({ intensities, locale, size = 240 }: Props) {
  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      // Merge amount m: 0 released (7 distinct) → 1 merged (one light). Restored STEP-6 cycle,
      // now sourced from the pure, unit-tested weeklyOrbMerge helper (identical values).
      const m = mergeAmount(t, reduceMotion);
      ...
      const absorbHi = absorbHiAt(m, reduceMotion);
      ...
      for (let i = 0; i < n; i++) {
        ...
        const ringR = ringBase * (1 + np.radOffset) * nodeGatherScale(m);
