...
/** `/[locale]/bty-arena/loading` — `<main>` 랜드마크 (라우트 세그먼트 로딩). */
export default function BtyArenaRouteLoadingShell() {
  const pathname = usePathname() ?? "";
  const locale = (pathname.startsWith("/ko") ? "ko" : "en") as Locale;
  const t = getMessages(locale).uxPhase1Stub;
  return (
    <main aria-label={t.arenaBtyArenaRouteSegmentLoadingMainRegionAria} className="min-h-screen bg-white">
