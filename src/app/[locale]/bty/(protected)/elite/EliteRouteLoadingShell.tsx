...
/** `/[locale]/bty/(protected)/elite/loading` — `<main>` 랜드마크 (라우트 세그먼트 로딩). */
export default function EliteRouteLoadingShell() {
  const pathname = usePathname() ?? "";
  const locale = (pathname.startsWith("/ko") ? "ko" : "en") as Locale;
  const t = getMessages(locale).bty;
  return (
    <main aria-label={t.eliteSuspenseMainRegionAria} className="min-h-screen bg-white">
