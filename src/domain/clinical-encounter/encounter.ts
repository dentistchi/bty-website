...
export type ClinicalEncounterCaseV2 = { caseId: string; version: string; chiefComplaint: string; patientFacts: EncounterFact[]; objectiveFindings: EncounterFact[]; tests: EncounterFact[]; media: EncounterFact[]; hiddenReference: { rubric: string[]; clinicianValidationStatus: "pending" | "validated" } };
...
export const CASE_001_ENCOUNTER: ClinicalEncounterCaseV2 = {
  ...
  objectiveFindings: [fact("fracture-extent", "exam"), fact("pulp-exposure", "exam", { objectiveResponse: "No visible pulp exposure is present." }), fact("mobility", "exam", { aliases: ["mobile", "is mobile", "check if mobile"] }), fact("displacement", "exam"), fact("probing", "exam"), fact("occlusion", "exam", { aliases: ["occlusal", "occlusal space"] })],
  ...
  media: [fact("radiograph", "imaging", { aliases: ["pa", "take a pa", "pa x ray", "periapical x ray"] }), fact("root-fracture", "imaging"), fact("alveolar-fracture", "imaging")],
...
export type EncounterInterpretation = { intent: EncounterIntent; canonicalIds: string[]; ambiguous: boolean };
...
const factIdsFor = (tokens: string[], facts: EncounterFact[]) => {
...
export function interpretEncounter(input: string, c = CASE_001_ENCOUNTER): EncounterInterpretation {
  ...
  const canonicalIds = intent === "imaging_request" || intent === "media_request" ? factIdsFor(tokens, c.media) : intent === "clinical_exam" ? factIdsFor(tokens, c.objectiveFindings) : intent === "diagnostic_test" ? requestedTestIds(tokens, c.tests) : intent === "patient_question" ? factIdsFor(tokens, c.patientFacts) : [];
  return { intent, canonicalIds, ambiguous: canonicalIds.length === 0 };
}
function unknownResponse(intent: EncounterIntent) { if (intent === "imaging_request" || intent === "media_request") return "No authored clinical image is available for this version."; if (intent === "clinical_exam" || intent === "diagnostic_test") return "That finding is not available in this case. Request an authored examination, test, or image."; return "I don't know that. Please ask about something I can report or request an examination, test, or image."; }
export function respondToEncounter(input: string, c = CASE_001_ENCOUNTER) {
  ...
  const facts = [...c.patientFacts, ...c.objectiveFindings, ...c.tests, ...c.media];
