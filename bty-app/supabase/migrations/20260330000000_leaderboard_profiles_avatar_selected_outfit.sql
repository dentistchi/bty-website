...
comment on function public.get_leaderboard_profiles(uuid[]) is
  'Returns profile rows for given user IDs for leaderboard display. SECURITY DEFINER so RLS does not restrict to current user. Includes avatar_selected_outfit_id for layer composition.';
