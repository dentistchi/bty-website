...
**다른 Cursor에 붙여넣을 한 줄 명령:**
```
bty-app/docs/CURSOR_AVATAR_GAME_ACCESSORIES_TASK.md 를 열고, "지시문 (전체 복사)" 안에 있는 대로 게임용 악세사리 35개 아이콘을 bty-app/public/avatars/accessories/ 에 id.png 로 그려서 저장해줘.
```
...
**저장 경로:** `bty-app/public/avatars/accessories/`
...
치과 악세사리(핸드피스, 치과거울, Explorer, Loupes, 고글 등)는 **이 Cursor(현재 창)** 에서 별도로 그리므로, 위 지시문에서는 **게임용만** 처리하면 됨.
