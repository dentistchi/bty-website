...
## 0. Ready Player Me 종료 (2026년 1월)

**Ready Player Me 서비스는 2026년 1월에 종료됩니다.** (RPM 측 이메일로 안내 확인됨.)  
신규 연동은 하지 않고, **외부 아바타 서비스(Avatar SDK/Loom 등) 연동도 보류.**  
**현재 정책:** **이니셜**(기본) + **본인이 지정하는 이미지**(URL 입력 또는 추후 업로드). 사용자가 핸드폰 등에서 생성한 아바타/사진 URL을 저장할 수 있으면 됨. `PATCH /api/arena/profile`로 `avatarUrl` 저장은 이미 구현됨.
