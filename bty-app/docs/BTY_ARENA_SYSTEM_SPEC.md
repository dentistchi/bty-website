...
### 시즌 종료 시 10% Carryover (구현)

- 시즌(리그) 구간은 30일. 새 시즌이 시작될 때(active league가 없어 새로 생성되는 순간) **기존 Seasonal XP의 10%만 남기고** 나머지는 리셋한다.
- 적용 대상: `weekly_xp` 테이블에서 `league_id IS NULL`인 행(현재 시즌 누적). `xp_total = FLOOR(xp_total * 0.1)` 로 한 번에 갱신.
- Core XP는 영향 없음. 리더보드·시즌 랭킹은 갱신된 Seasonal XP 기준.
...
- **언제**: 해당 Code 안에서 **Tier 25 도달** + **당 시즌 리더보드 상위 5%(Elite)** 일 때 1회 “리네임 기회” 부여.  
  (구현: Tier ≥ 25, Code당 미사용, Code 0–5, **Seasonal XP 기준 순위가 상위 5% 이내**일 때만 가능)
...
5. **Elite 조건**: 리네임 기회는 **당 시즌 리더보드(Seasonal XP) 상위 5%**에 들어가 있을 때만 부여·사용 가능. (상위 5% = rank ≤ ceil(0.05 × 참여자 수))
...
### 시나리오 · 윤리적 선택 · Elite 전용

- **윤리적 선택 보상**: 선택지의 `hiddenDelta.integrity > 0`(정직/윤리 쪽 선택)일 때 **소량 보너스 XP** 지급. (구현: +5 XP)
- **Tier 75(Elite) 전용 시나리오**: 시나리오에 `minTier: 75`를 두면 **Tier 75 이상** 유저만 해당 시나리오가 랜덤 풀에 포함됨. 상위 구간 전용 “하드” 콘텐츠로 활용.
...
| 시즌 리셋 | 30일 시즌 종료 시 **10% carryover** 후 나머지 리셋 (run_season_carryover 호출) |
