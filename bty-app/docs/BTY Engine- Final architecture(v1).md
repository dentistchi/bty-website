...
> Status: LOCKED · 제품/코드/팀 공유 기준 문서  
...
| 트리거 | 조건 | 타입 |
|--------|------|------|
| Immediate | `exit` + intensity 3 | Forced QR |
| Accumulation | same axis `exit` ≥ 3 (최근 5회) | QR |
| Convergence | 3개 이상 axis에서 `exit` | QR |
| Integrity Gap | `entry` 많음 + 실행 없음 | Forced QR |
...
```
Action Request → Execution → Approval (Relational) → XP
```
...
1. Scenario          압박 발생
2. Choice            행동 vs 회피
3. Pattern Acc.      axis pressure 증가
4. Trigger Engine    행동 필요 판단
5. QR Action         행동 실행 → 관계 검증
6. Re-exposure       동일 상황 재진입
7. Change Detection  선택 변화 확인
8. Growth Update     AIR / LRI / XP 반영
```
