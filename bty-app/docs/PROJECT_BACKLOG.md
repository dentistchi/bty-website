...
## 10. Center·Arena·챗봇·대시보드 (커맨더 백로그)

**통합 문서**: `docs/COMMANDER_BACKLOG_AND_NEXT.md` — 분류·Cursor 역할·"다음에 뭘 할지" 리스트.

**Center 페이지**  
- **명세**: `docs/CENTER_PAGE_IMPROVEMENT_SPEC.md`  
- CTA 통합·재로그인 버그, "챗으로 이어하기" 동작, 5문항→50문항 순서, 회복 탄력성 그래프 매일 트렉, EN/KO 플로우 통일, 50문항 정성 플로우, 아늑한 방 톤. **Center부터 우선 진행.**

**챗봇 플로팅**  
- 전역에서 오른쪽 아래 플로팅 챗봇 **일단 전부 제거**. Center·Foundry에는 챗 기능이 있으므로 별도 창 불필요. 연결 끊김 이슈 별도 기록.

**Arena 한국어**  
- 한국어 선택 시 시뮬레이션이 영어로 나오는 문제 확인. 새 포맷 미반영이면 **Cursor 4**에 위임.

**Arena 아바타**  
- **`docs/ARENA_CODENAME_AVATAR_PLAN.md`** §3: 얼굴·몸 레이어 분리, 옷 선택 시 **옷 이미지 노출** + **그 옷을 입은 캐릭터** 표시. Cursor 4 권장.

**대시보드 코드네임**  
- 코드네임 **단계·수준 설명** somewhere. 아이디어: **마우스 오버 시 팝업(툴팁)** 으로 노출.

**리더보드**  
- **팀별**: 팀(역할/지점)별로 **특정 수치만** 노출 (BTY_ARENA_SYSTEM_SPEC §4, ENGINE TII 등).  
- **개인별**: Near Me — 내 위 5명 + 나 + 아래 5명 (BACKLOG §1 완료).
