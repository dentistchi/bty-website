...
| 일자 | 담당 | 완료 § | 변경 파일 요약 |
|------|------|--------|----------------|
| 2026-03-03 | Cursor 4 | §6·§7 (render-only 보완) | AvatarComposite: render-only 주석(API/도메인 URL만 표시). Dashboard: avatarAccessoryUrls = core.currentOutfit.accessoryIds → getAccessoryImageUrl, AvatarComposite에 전달. OutfitCard 주석. lint Exit 0. |
| 2026-04-01 | Cursor 1 | §1, §3, §6, §7 (DB + API) | 마이그레이션: `supabase/migrations/20260401000000_arena_profiles_avatar_accessories_and_lock.sql` (avatar_accessory_ids, avatar_outfit_updated_at, avatar_accessories_updated_at, enforce_avatar_character_lock 트리거). API: `src/app/api/arena/profile/avatar/route.ts` (GET AvatarUiResponse: avatar + allowed.outfits/accessorySlots, PATCH theme/outfitKey/accessoryKeys 검증). 도메인: `src/lib/bty/arena/avatarOutfits.ts` (getAllowedOutfitsForLevel, accessorySlotsFromTier, AllowedOutfitEntry). |
| 2026-03-03 | Cursor 4 | §6, §7 (검증·테스트) | 타입: `src/types/arena.ts` (AvatarCompositeKeys, PatchAvatarRequest, AvatarUiResponse). API: `src/app/api/arena/profile/avatar/route.ts` — 타입 import를 `@/types/arena`로 변경. 단위 테스트: `src/lib/bty/arena/avatarOutfits.test.ts` — getAllowedOutfitsForLevel(theme, levelId), getOutfitById(theme, outfitId) 유효성, accessorySlotsFromTier(tier), accessoryKeys.length ≤ slots 검증 규칙. E2E는 선택 항목으로 생략. |
| 2026-03-03 | Cursor 3 | §2, §5, §6, §7 (avatarAssets + AvatarComposite + UX) | `src/lib/bty/arena/avatarAssets.ts`: characterAssetMap, outfitAssetMap, accessoryAssetMap, resolveAvatarUrls. `src/components/bty-arena/AvatarComposite.tsx`: 레이어 base→outfit→accessory, lazy, aria. `src/hooks/useAvatar.ts`: GET/PATCH /api/arena/profile/avatar. `src/components/bty-arena/OutfitCard.tsx`: OutfitCard. `src/app/[locale]/bty/(protected)/profile/avatar/`: page.tsx + AvatarSettingsClient.tsx (Theme 토글, Outfit Gallery, 저장). 리더보드·대시보드: AvatarComposite + lazy. |
| 2026-03-03 | — | §6·§7 리더보드 응답 확장 | 도메인 `profileToAvatarCompositeKeys`(avatarOutfits). GET /api/arena/leaderboard 각 행에 `avatar: { characterKey, theme, outfitKey, accessoryKeys }`. RPC `get_leaderboard_profiles`에 avatar_accessory_ids. 마이그레이션 20260402000000. |
| — | — | — | — |
