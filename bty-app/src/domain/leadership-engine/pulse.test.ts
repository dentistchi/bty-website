...
describe("computePulse14d", () => {
  it("empty input -> pending (hasPulse false, pulseMean 0)", () => {
    expect(computePulse14d([], asOf)).toEqual({ pulseMean: 0, hasPulse: false });
  });

  it("all records outside 14d window -> pending", () => {
    const out = [rec(5, daysBefore(15)), rec(4, daysBefore(30))];
    expect(computePulse14d(out, asOf)).toEqual({ pulseMean: 0, hasPulse: false });
  });

  it("single in-window value 5 -> mean 5", () => {
  ...
  it("single in-window value 1 -> mean 1", () => {
  ...
  it("raw mean of multiple (2,4 -> 3)", () => {
    const r = [rec(2, daysBefore(2)), rec(4, daysBefore(5))];
    expect(computePulse14d(r, asOf)).toEqual({ pulseMean: 3, hasPulse: true });
  });
  ...
  it("only in-window records contribute to mean (in 4 / out 1 -> 4)", () => {
