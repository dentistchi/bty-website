...
const ENCOURAGEMENT =
  "오늘도 한 걸음 나아갔네요. 실패해도 다시 온 것만으로도 충분해요. 그대로만 가도 돼요.";
...
export function MissionCard({
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-foundry-ink/40 z-50" />
        <Dialog.Content
          className={cn(
            "fixed left-1/2 top-1/2 z-50 w-[min(96vw,28rem)] max-h-[90vh] -translate-x-1/2 -translate-y-1/2",
            "rounded-2xl border border-foundry-purple-muted bg-foundry-white shadow-xl",
            "overflow-y-auto"
          )}
        ...
        >
          <div className="p-6 sm:p-8 space-y-6">
            <div className="flex justify-between items-center">
              <Dialog.Title className="text-xl font-semibold text-foundry-purple-dark">
                Day {day} · 오늘의 미션
              </Dialog.Title>
              <Dialog.Close
                className="rounded-lg p-2 text-foundry-ink-soft hover:bg-foundry-purple-muted/50 hover:text-foundry-ink transition-colors"
                aria-label="닫기"
            ...
            <AnimatePresence mode="wait">
              {showEncouragement ? (
                ...
                >
                  <p className="text-lg font-medium text-foundry-purple-dark">
                    {ENCOURAGEMENT}
                  </p>
                  <Dialog.Close
                ...
                >
                  {/* 1) 오늘의 읽을거리 */}
                  <section>
                    <h3 className="text-sm font-medium text-foundry-ink-soft mb-2">
                      오늘의 읽을거리
                    </h3>
                    <p className="text-foundry-ink leading-relaxed">
                      {content.reading}
                    </p>
                  </section>
                  ...
                  {/* 2) 오늘의 미션 (체크박스) */}
                  <section>
                    <h3 className="text-sm font-medium text-foundry-ink-soft mb-3">
                      오늘의 미션
                    </h3>
                      ...
                      {content.missions.map((mission, idx) => (
                        <label
                          ...
                          className={cn(
                            ...
                            mode === "edit" && "cursor-pointer hover:bg-foundry-purple-muted/30",
                            checks.includes(idx)
                              ? "border-foundry-purple/40 bg-foundry-purple/5"
                              : "border-foundry-purple-muted"
                          )}
                        >
                          <Checkbox.Root
                            ...
                            className={cn(
                              ...
                              checks.includes(idx)
                                ? "bg-foundry-purple border-foundry-purple"
                                : "border-foundry-purple-muted bg-white"
                            )}
                          >
                          ...
                          <span className="text-foundry-ink text-sm leading-relaxed">
                            {mission}
                          </span>
                        </label>
                      ))}
                  ...
                  {mode === "edit" && (
                    <section>
                      ...
                      <p className="text-xs text-foundry-ink-soft mb-2">
                        오늘의 마음을 적어보세요. AI가 공감해줄 거예요.
                      </p>
                      <div className="rounded-xl border border-foundry-purple-muted p-4 bg-foundry-purple-muted/10">
                        <SafeMirror
                          locale="ko"
                          theme="sanctuary"
                  ...
                  {mode === "view" && initialReflection && (
                    <section>
                      ...
                      <div className="rounded-xl border border-foundry-purple-muted p-4 bg-foundry-purple-muted/10">
                        <p className="text-foundry-ink text-sm whitespace-pre-wrap">
                          {initialReflection}
                        </p>
                      </div>
                    </section>
