...
export function userMessageForOAuthCallbackError(
  ...
  const c = code.trim();
  if (c === "missing_code") {
    return locale === "ko" ? "인증 정보가 없습니다. 다시 시도해 주세요." : "Missing sign-in code. Please try again.";
  }
  ...
  return locale === "ko" ? "문제가 발생했습니다. 잠시 후 다시 시도해 주세요." : "Something went wrong. Please try again.";
}
