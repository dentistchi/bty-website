...
/** `/[locale]/bty/(protected)/dojo/loading` — `<main>` 랜드마크 (Suspense 라우트 로딩). */
export default function DojoRouteLoadingShell() {
  const pathname = usePathname() ?? "";
  const locale = (pathname.startsWith("/ko") ? "ko" : "en") as Locale;
  const t = getMessages(locale).bty;
  return (
    <main aria-label={t.dojoSuspenseMainRegionAria} className="min-h-screen bg-white">
