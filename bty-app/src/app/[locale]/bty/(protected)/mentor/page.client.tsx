...
const OPENING_KO = "어떤 리더십 상황에 대해 이야기할까요? 팀, 의사결정, 갈등 — 무엇이든 편하게 말씀해 주세요.";
const OPENING_EN = "What leadership situation would you like to talk about? Teams, decisions, conflict — anything goes.";
...
const MENTOR_UI = {
  ...
  en: {
    ...
    restartChat: "Start new conversation",
    eliteRequestCta: "Request 1:1 session",
  },
} as const;
...
export default function MentorPage() {
    ...
    async (role: "user" | "assistant", content: string) => {
      ...
      try {
        await fetch("/api/me/conversations", {
          ...
          body: JSON.stringify({ channel: "mentor", sessionId, topic: "leadership", role, content }),
        });
      } catch {
        // ignore
      }
    },
    [rememberMentor, sessionId]
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setMessages([{ role: "chi", text: isEn ? OPENING_EN : OPENING_KO }]);
    })();
  }, [prefsLoaded, rememberMentor, isEn]);
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "chi", text: isEn ? "Connection was unstable. Please resubmit." : "연결이 잠깐 불안정했네요. 다시 말해주실래요?" },
      ]);
    } finally {
      setSending(false);
    }
      ...
      <AuthGate>
        ...
        >
          <div className="max-w-2xl mx-auto px-4 py-6 sm:py-10 min-h-screen flex flex-col">
            <div style={{ flex: 1, paddingTop: 48 }} aria-busy="true" aria-label={isEn ? "Loading…" : "불러오는 중…"}>
      ...
      >
        <div className="max-w-2xl mx-auto px-4 py-6 sm:py-10 min-h-screen flex flex-col">
          <header className="text-center mb-8">
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                {isElite && (
                  <span className="inline-flex items-center rounded-full bg-mentor-wood/20 px-2.5 py-0.5 text-xs font-medium text-mentor-wood border border-mentor-wood-soft/40">
                    {t.eliteMentorBadge}
                  </span>
                )}
              </div>
            ...
            {deleting && (
              <div className="mt-3" aria-busy="true" aria-label={isEn ? "Deleting conversation history" : "대화 기록 삭제 중"}>
          ...
          <MentorConversationHistory
            ...
            topicLabels={{ leadership: isEn ? "Leadership" : "리더십" }}
          />
          ...
          <footer className="mt-4 text-center text-sm text-mentor-ink-soft space-x-3">
            ...
            <span>·</span>
            <Link href={`/${locale}/bty/dashboard`} className="text-mentor-wood hover:underline" aria-label={isEn ? "Go to Dashboard" : "대시보드로 가기"}>
              {t.toDashboard}
            </Link>
            {isElite && (
              <>
                <span>·</span>
                <Link href={`/${locale}/bty/elite`} className="text-mentor-wood hover:underline" aria-label={t.eliteRequestCta}>
                  {t.eliteRequestCta}
                </Link>
              </>
            )}
