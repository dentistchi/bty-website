...
/** `/[locale]/bty/(protected)/mentor/loading` — `<main>` 랜드마크 (라우트 세그먼트 로딩). */
export default function MentorRouteLoadingShell() {
  const pathname = usePathname() ?? "";
  const locale = (pathname.startsWith("/ko") ? "ko" : "en") as Locale;
  const t = getMessages(locale).bty;
  return (
    <main aria-label={t.mentorSuspenseMainRegionAria} className="min-h-screen bg-white">
