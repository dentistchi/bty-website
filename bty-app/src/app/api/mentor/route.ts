...
function getSafetyValveMessage(lang: "en" | "ko"): string {
  if (lang === "en") {
    return "Hold on—you look really worn out. Before we work on skills, it might help to rest in Center for a bit. Would you like to go there and check in with how you're feeling?";
  }
  return "잠깐만요, 지금 많이 지쳐 보여요. 기술을 배우는 것보다 마음을 돌보는 게 먼저인 것 같네요. 우리 Center로 가서 잠시 쉬고 올까요? 거기서 당신의 마음 상태를 체크해보고 오세요.";
}

function getMentorFallbackMessage(lang: "en" | "ko"): string {
  if (lang === "en") {
    return "What's been on your mind lately? If you share a bit more, we can think it through together.";
  }
  return "요즘 어떤 부분이 가장 고민되나요? 구체적으로 말해주시면 같이 생각해볼게요.";
}
...
function buildOpenAIMessages(
  ...
  if (base[0]?.role === "system" && typeof base[0].content === "string") {
    ...
    base[0] = {
      role: "system",
      content: langPrefix + DR_CHI_PHILOSOPHY + "\n\n" + base[0].content,
    };
  }
